.PHONY: help check test lint build clean-dist clean-testdata clean-all data data-example-mismatch data-benchstat-repeat-fast data-gotestbench e2e e2e-readme-pipelines e2e-benchstat e2e-gotestbench e2e-ab e2e-insufficient

VERDICT := ./dist/verdict

help:
	@printf 'Development targets (run from the repository root):\n'
	@printf '  make check                Run all validation gates: test, lint, and e2e.\n'
	...
	@printf '  make data-gotestbench     Regenerate the raw go test -bench fixture.\n'
	...
	@printf '  make e2e-gotestbench      Check raw go test -bench stdin parsing.\n'
...
data: data-example-mismatch data-benchstat-repeat-fast data-gotestbench
